#!/bin/sh

echo "[❗] Clearing cache"
rm -rf .yarn/cache .nx/cache .angular node_modules yarn.lock dist

if [ ! -f ./yarn.lock ]; then
    echo "[❗] yarn.lock File not found!"
    echo "[📄] creating new lock file.."
    touch yarn.lock
fi

echo "[⚡] Setting Environment variable"
export CODEARTIFACT_AUTH_TOKEN=`aws codeartifact get-authorization-token --domain fss --domain-owner 736483234466 --region ap-south-1 --query authorizationToken --output text --no-verify-ssl`

echo "[📝] Setting yarn config"
yarn config set npmRegistryServer https://fss-736483234466.d.codeartifact.ap-south-1.amazonaws.com/npm/npm-store/
yarn config set 'npmRegistries["https://fss-736483234466.d.codeartifact.ap-south-1.amazonaws.com/npm/npm-store/"].npmAuthToken' "${CODEARTIFACT_AUTH_TOKEN}"
yarn config set 'npmRegistries["https://fss-736483234466.d.codeartifact.ap-south-1.amazonaws.com/npm/npm-store/"].npmAlwaysAuth' "true"

echo "[⚡] Running yarn"
yarn install

echo "[⚡] Extracting required artifacts.."
cd .yarn/cache

#################################################################
###################  All Clay Libs ##############################
#################################################################

fVar=$(find * -type f -name '@clay-ui-commons-npm-*.*\.zip');
unzip -o ${fVar} "node_modules/@clay/ui-commons/assets/*" -d "./../../"
unzip -o ${fVar} "node_modules/@clay/ui-commons/styles/*" -d "./../../"
unzip -o ${fVar} "node_modules/@clay/ui-commons/esm2022/**/*.mjs" -d "./../../"

fVar=$(find * -type f -name '@clay-tailwind-preset-npm-*.*\.zip');
unzip ${fVar} "node_modules/@clay/tailwind-preset/tailwind.config.js" -d "./../../"
unzip -o ${fVar} "node_modules/@clay/tailwind-preset/styles/*" -d "./../../"
unzip -o ${fVar} "node_modules/@clay/tailwind-preset/tailwind/*" -d "./../../"

fVar=$(find * -type f -name '@clay-ui-components-npm-*.*\.zip');
unzip -o ${fVar} "node_modules/@clay/ui-components/esm2022/**/*.mjs" -d "./../../"

fVar=$(find * -type f -name '@clay-app-shell-npm-*.*\.zip');
unzip -o ${fVar} "node_modules/@clay/app-shell/styles/*" -d "./../../"
unzip -o ${fVar} "node_modules/@clay/app-shell/esm2022/**/*.mjs" -d "./../../"

#####################################################################
#####################################################################
